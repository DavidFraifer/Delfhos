...
# --- Cell splitting (Jupyter-style sequential execution) ----------------------

@dataclass
class _Cell:
    """One executable unit = one top-level statement of the generated script.

    Cells run in order against the shared namespace, so each successful cell is
    a free state checkpoint. Cells containing an ``await`` are the meaningful
    recovery boundaries (tool calls = expensive/side-effecting work).
    """
    source: str
    has_await: bool
...
# Generated scripts may use top-level `await`/`return`, which only parse inside a
# function. We wrap solely to parse, then recover each statement's source.
...
def split_into_cells(code: str) -> List[_Cell]:
    """Split a generated script into ordered cells (one per top-level statement).

    A top-level ``return X`` is rewritten to ``__cell_result__ = (X)`` and marks
    the cell terminal, preserving the "explicit return becomes the result"
    behaviour of the previous async-function wrapper. Raises SyntaxError for
    genuinely invalid code (handled by the caller, same as before).
    """
    src = code if code.endswith("\n") else code + "\n"
    wrapped = _PARSE_WRAP + textwrap.indent(src, "    ")
    func = ast.parse(wrapped).body[0]               # the synthetic async def
    ...
    for node in func.body:
        if isinstance(node, ast.Return):
            ...
            cells.append(_Cell(source=f"__cell_result__ = ({value})",
                               has_await=_contains_await(node), terminal=True))
        else:
            segment = ast.get_source_segment(wrapped, node)
            if segment is None:
                continue
            cells.append(_Cell(source=textwrap.dedent(segment),
                               has_await=_contains_await(node)))
    return cells or [_Cell(source=code, has_await=_contains_await(func))]
...
class PythonExecutor:
        ...
        try:
            ...
            with redirect_stdout(stdout_capture), redirect_stderr(stderr_capture):
                result = await asyncio.wait_for(
                    self._execute_cells(code, self.namespace),
            ...
            return {
                ...
                "execution_time": execution_time,
                "output_files": dict(getattr(self, "_output_registry", {})),
                **self._cell_failure_info(),
            }

    def _cell_failure_info(self) -> Dict[str, Any]:
        """Cell-level checkpoint info for the retry path (empty if unavailable).

        ``pending_source`` is the failed cell plus everything after it — the only
        work left to do. Earlier cells already ran and must not be repeated.
        """
        cells = getattr(self, "_cells", None)
        idx = getattr(self, "_failed_cell_index", None)
        if not cells or idx is None:
            return {}
        return {
            "failed_cell_index": idx,
            "cells_total": len(cells),
            "pending_source": "\n".join(c.source for c in cells[idx:]),
        }

    async def _execute_cells(self, code: str, namespace: dict) -> Any:
        """
        Execute the generated script cell by cell against the shared namespace.

        Each top-level statement is one cell, compiled with top-level-await
        support and run in order. Because names bind straight into ``namespace``,
        every successful cell is a state checkpoint — no frame-locals rescue
        needed. On failure, cells before the failing one are already applied and
        are never re-run, so their side effects aren't duplicated on retry.
        """
        cells = split_into_cells(code)

        # Heuristic: if an async entrypoint was defined but never awaited, run it.
        for entry in ("main", "run"):
            if f"async def {entry}" in code and f"await {entry}(" not in code:
                cells.append(_Cell(source=f"await {entry}()", has_await=True))
                break

        self._cells = cells
        self._failed_cell_index = None

        # Protected names (builtins, tool libraries, special vars) must survive a
        # cell that shadows them, or later cells / retries lose access to them.
        protected = {k: namespace[k] for k in getattr(self, "_baseline_keys", set())
                     if k in namespace}
        ...
        for i, cell in enumerate(cells):
            try:
                ...
                coro = eval(compiled, namespace)   # coroutine iff cell has top-level await
                if coro is not None:
                    await coro
                cell.status = "ok"
            except Exception as e:
    ...
    def _format_error(self, exception: Exception, stderr: str) -> str:
        ...
        # Filter to show only user code (not internal executor code); generated
        # cells appear as "<cell N>" frames, which we keep.
        filtered_tb = [line for line in tb_lines if "python_executor.py" not in line]
